...
function head({ lang, title, description, path, image, jsonld = [], ogType = "website", publishedTime }) {
  ...
  const gtm = site.tracking.gtm
    ...
    : "";
  const gtagId = site.tracking.ga4 || site.tracking.googleAds;
  const gtagConfigs = [site.tracking.ga4, site.tracking.googleAds]
    .filter(Boolean)
    .map((id) => `gtag('config','${id}');`)
    .join("");
  const ga4 = gtagId
    ? `<script async src="https://www.googletagmanager.com/gtag/js?id=${gtagId}"></script><script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());${gtagConfigs}</script>`
    : "";
